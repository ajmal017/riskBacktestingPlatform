...
def mytable():
    ...
    for i in accountInfonew :
          ...
          for j in i:
             temp.append(float(j))
             if len(total)>6 :
                total[m] =round((total[m] +temp[m]),2)
             else :
...
def producer():
    ...
    #回测结果
    col=db[title]
    data=xlrd.open_workbook(r'C:\vnpy-master190102\vnpy-master\vnpy\trader\app\ctaStrategy\resultsDF.xlsx')
    table=data.sheets()[0]
    ...
    data=xlrd.open_workbook(r'C:\vnpy-master190102\vnpy-master\vnpy\trader\app\ctaStrategy\nvDF.xlsx')
    table=data.sheets()[0]
    rowstag=table.row_values(0)
    nrows=table.nrows
    returnData={}
    ...
    data=xlrd.open_workbook(r'C:\vnpy-master190102\vnpy-master\vnpy\trader\app\ctaStrategy\varDF.xlsx')
    table=data.sheets()[0]
    rowstag=table.row_values(0)
    nrows=table.nrows
    returnData={}
    ...
    data=xlrd.open_workbook(r'C:\vnpy-master190102\vnpy-master\vnpy\trader\app\ctaStrategy\marginDF.xlsx')
    table=data.sheets()[0]
    rowstag=table.row_values(0)
    nrows=table.nrows
    returnData={}
    ...
    data=xlrd.open_workbook(r'C:\vnpy-master190102\vnpy-master\vnpy\trader\app\ctaStrategy\HedgeResult.xlsx')
    table=data.sheets()[0]
    rowstag=table.row_values(0)
    nrows=table.nrows
    returnData={}
...
def writeCTP_connect_json(param):            
    ...
    path="C:/vnpy-master190102/vnpy-master/examples/WebTrader/CTP_connect.json"
    with open(path, 'w') as f :
	json.dump(param,f)
